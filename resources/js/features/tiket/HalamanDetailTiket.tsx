...
export function HalamanDetailTiket({ id }: { id: string }) {
    ...
    const handleKirimBalasan = async () => {
        ...
        try {
            const res = await api.tiket.kirimBalasanTiket(
                id,
                balasan.trim(),
                status,
            );
            setTiket(res);
            toast.success(
                'Tanggapan resmi admin berhasil dikirim & status diperbarui.',
            );
        } catch (e) {
            const pesan =
                e instanceof KesalahanApi
    ...
    if (!tiket) {
        return (
            <div className="py-16 text-center">
                <AlertCircleIcon className="mx-auto mb-3 h-12 w-12 text-muted-foreground opacity-60" />
                <p className="mb-1 text-lg font-medium text-foreground">
                    Tiket Tidak Ditemukan
                </p>
                <p className="mb-4 text-sm text-muted-foreground">
                    Laporan atau tiket bantuan dengan ID ini tidak tersedia di
                    database.
                </p>
                <Button
                    variant="outline"
        ...
        <div className="mx-auto max-w-5xl space-y-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <Button
                    ...
                    onClick={() => router.visit('/tiket')}
                >
                    <ArrowLeftIcon className="mr-1.5 h-4 w-4" /> Kembali ke
                    Daftar Tiket
                </Button>
                ...
                <CardContent className="p-6">
                    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                        <div className="space-y-1">
                            ...
                            <h1 className="pt-1 text-xl font-bold tracking-tight text-foreground">
                                {tiket.subjek}
                            </h1>
                            <p className="flex items-center gap-1.5 pt-0.5 text-xs text-muted-foreground">
                                <CalendarIcon className="h-3.5 w-3.5" />
                                Dikirim pada{' '}
                                {new Date(tiket.dibuatPada).toLocaleString(
                                    'id-ID',
                                    {
                                        day: 'numeric',
                                        month: 'long',
                        ...
                        <CardContent className="space-y-6 p-6">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 space-y-1.5">
                                    ...
                                    <div className="rounded-xl border bg-muted/60 p-4 text-sm leading-relaxed whitespace-pre-wrap">
                                        {tiket.pesan}
                                    </div>
                                </div>
                            </div>
                            ...
                            {tiket.balasanAdmin && (
                                <div className="flex items-start gap-4 pt-2">
                                    <Avatar className="h-10 w-10 border border-emerald-500/30 shadow-sm">
                                        <AvatarFallback className="bg-emerald-500/10 font-bold text-emerald-600">
                                    ...
                                    <div className="flex-1 space-y-1.5">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm font-semibold text-emerald-950 dark:text-emerald-300">
                                                    {tiket.adminNama ||
                                                        'Admin Support'}
                                                </span>
                                                <Badge
                                                    variant="default"
                                                    className="bg-emerald-600 px-2 py-0 text-[10px] hover:bg-emerald-600"
                                                >
                                                    Tim Resmi
                                                </Badge>
                                            </div>
                                        ...
                                        <div className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm leading-relaxed whitespace-pre-wrap text-emerald-950 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-200">
                                            {tiket.balasanAdmin}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4 p-6">
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                <div>
                                    ...
                                    <div className="mt-0.5 flex items-center justify-between">
                                        <p className="max-w-[180px] truncate font-mono text-xs text-foreground">
                                            {tiket.toko.email}
                                        </p>
                                        <Button
                                            variant="ghost"
                            ...
                            <Button
                                ...
                                className="w-full"
                                onClick={() =>
                                    router.visit(`/pengguna/${tiket.toko.id}`)
                                }
                            >
                                <UserIcon className="mr-2 h-4 w-4" /> Detail
                                Pengguna Toko
                            </Button>
                        </CardContent>
                    </Card>
